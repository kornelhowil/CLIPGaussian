...
from model.vegas.gaussian_renderer import render, network_gui
from model.vegas.scene import Scene
from model.vegas.models import gaussianModel, optimizationParamTypeCallbacks
from model.vegas.utils.general_utils import safe_state
from model.vegas.utils.image_utils import psnr
from model.vegas.utils.loss_utils import l1_loss
...
def training(gs_type, dataset, opt, pipe, args):
    ...
    # Restrict optimization to only color features for stylization
    # This matches the restricted training_setup logic from the prototype
    l = [
    ...
    with torch.no_grad():
        for cam in scene.getTrainCameras().copy():
            ...
            clip_features = clip_model.encode_image(clip_normalize(gt_image.unsqueeze(0)))
...
def training_report(tb_writer, iteration, loss, l1_loss, elapsed, testing_iterations, scene, renderFunc,
                    renderArgs):
    if tb_writer:
    ...
    if iteration in testing_iterations:
        ...
        print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} Num Points {}".format(iteration, config['name'], l1_test, psnr_test, num_gaussians))
        if tb_writer:
